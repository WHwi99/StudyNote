...
-- 06_join
-- 두개 이상의 테이블을 공통 된 컬럼 값을 기준으로 행을 결합하여 하나의 결과 집합을 만드는 SQL 연산
-- 일반적으로 두 테이블은 공통 된 의미를 가지는 컬럼(키)을 기준으로 결합하여 이를 join 조건이라고 한다.
...
FROM
    tbl_menu a
-- INNER ( 일반적으로 많이 생략해서 사용 )
JOIN tbl_category b ON (a.category_code = b.category_code);

-- employeedb의 employee + department
-- ON은 join조건 컬럼명이 같아도 되고 달라도 된다 두 테이블 간의 관계를 조건식으로 직접 작성한다.
SELECT
    a.emp_name,
    b.dept_title
FROM
...
-- INNER JOIN에서는 조건이 일치하는 행만 반환하므로 employee에서 dept_code가 null인 사원은 포함하지 않는다.
-- 일치하지 않는 행도 포함하여 조회하려면 outer join을 사용해야 한다.

-- using을 이용한 inner join
-- 두 테이블에서 join 기준 컬럼명이 동일한 경우에 사용이 가능하다.
-- 결과 집합에서도 해당 컬럼 하나만 표시된다.
SELECT
    a.menu_name,
    b.category_name
FROM
...
--   예를 들어 a 테이블의 행이 10개 b 테이블의 행이 5개면 10x5 = 50개의 행이 반환 된다.
SELECT
    a.menu_name,
    b.category_name
FROM
...
-- 각 테이블 사이의 관계를 순차적으로 join조건에 작성한다.
