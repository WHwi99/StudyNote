...
-- 2개 이상의 컬럼을 기준으로 그룹화
-- 각 컬럼 값 조합이 같은 행끼리 하나의 그룹으로 묶인다.
SELECT
    menu_price,
    category_code,
    COUNT(*) as menu_count
...
-- 3. rollup
-- with rollup을 사용하면 그룹화 집계 결과와 함께 중간 소계 또는 전체 총합 행이 추가 된다.
-- 이 때, 총합 또는 소계 행의 그룹 컬럼 캆은 NULL으로 표시 될 수 있다.

-- 컬럼 한 개를 그루핑 했을 경우 rollup
-- 마지막에 전체 메뉴 가격 총합 행을 조회한다. 
SELECT
    category_code,
    SUM(menu_price) as total_price
FROM
