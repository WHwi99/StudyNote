...
-- a or b : a와 b 중 하나라도 만족 할 경우 true
-- 주문 가능하거니 category_code가 4에 해당하는 메뉴
SELECT
    menu_code,
    menu_name,
    menu_price,
...
-- and, or 연산자 중에서 우선 순위가 높은 연산자는? => and
-- 만역 or 연산자를 먼저 사용하고 싶다면 소괄호를 활용해서 미리 묶어줘야 한다.
...
FROM
    tbl_menu
WHERE
    menu_name LIKE '_마늘%'; -- "_"는 자리차지

-- '_'나 '%'와 같은 기호를 실제로 검색하고 싶은 경우
-- LIKE '%\_%' 또는 LIKE '%!_%' ESCAPE '!'와 같은 방식으로 ESCAPE 해야한다.

-- NOT LIKE 처럼 NOT과 조합 가능
SELECT
    *
FROM
    tbl_menu
...
-- QUIZ) 메뉴 가격이 5000원 이상이고, 카테고리 코드가 10이며
...
FROM
    tbl_menu
WHERE
    menu_price >= 5000 AND category_code = 10 AND
    menu_name LIKE "%갈치%";
