...
-- 1. UNION
-- 두 개 이상의 select 문의 결과를 결합한 뒤 중복 된 레코드(행)를 제거하여 반환
SELECT
    menu_code,
    menu_name,
    menu_price,
...
-- 3. INTERSECT (8.0.31 ver 이상에서 추가)
-- 두 select문의 결과 중 공통 되는 레코드만 반환한다.
...
-- 4. EXCEPT
-- 첫번째 select문의 결과에서 두번째 select문의 결과에 포함 되는 레코드를 제외한 결과 반환
