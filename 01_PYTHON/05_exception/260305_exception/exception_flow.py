...
def demo_else_finally_file():
    ...
    try:
        ...
        # f.close() # => FileNotFoundError: [Errno 2] No such file or directory: 'data\\no_file.txt'
        ...
        if f is not None and not f.closed:
            f.close()
        print(f"[finally] 파일 핸들 정리 완료")
    # f.close()
    
class ScoreRangeError(Exception):   #Exception은 이미 정의 된 객체
    """
...
def validate_score(score):
    ...
    if not isinstance(score, (int, float)):
        raise TypeError("점수는 숫자여야 합니다.")  # raise => 예외를 발생 시킨다.
    if not (0 <= score <= 100):
        raise ScoreRangeError("점수는 0~100 사이여야 합니다.")
    
def to_grade(score):
    # 전달 받은 숫자의 유효성 검사를 먼저 수행한다.
