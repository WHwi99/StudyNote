...
def demo_import_styles():
    ...
    import mymodule as mm   # 별칭(alias)을 사용하여 import
    print(mm.VERSION)
    print(mm.add(5,6))
    ...
    from mymodule import add as a, multiply as m
    print(a(10,20))
    print(m(30,40))
...
# 표준 라이브러리 맛보기
def demo_stdlib_module():
    '''
    표준 라이브러리란?
    파이썬을 설치하면 기본으로 함께 제공 되는 공식 모듈/패키지 묶음이다.
    별도의 설치 없이 바로 import해서 사용할 수 있다.
    '''
    # math : 수학 상수/함수 제공
    import math
    ...
    # dir() : 모듈 내부에 어떤 함수/속성이 있는지 확인 할 수 있다.
    # print(dir(math))
...
'''
모듈을 모아놓은 파일 => 패키지
패키지임을 인식하게 하기 위해서 init.py를 생성하여 구별한다.
'''

'''
1) 파이썬 파일을 직접 실행하면 __name__값은 "__main__"으로 설정 된다.
지금 실행의 시작점이 이 파일이다 라는 뜻이다.
2) 파이썬 파일을 import해서 사용하면 __name__ 값이 "파일명"으로 설정 된다.
